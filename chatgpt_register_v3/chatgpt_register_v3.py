"""
ChatGPT batch registration tool v3.0.
支持多种邮箱服务，复用v2注册流程
"""
...
from lib.email_service import create_email_service
...
from chatgpt_register_v2.lib.config import as_bool
...
def main():
    """Run the CLI."""
    parser = argparse.ArgumentParser(description="ChatGPT 批量自动注册工具 v3.0")
    ...
    print("=" * 60)
    print(" ChatGPT 批量自动注册工具 v3.0")
    print(f" 使用 {mail_provider} 临时邮箱")
    print("=" * 60)

    total_accounts = args.num
    max_workers = args.workers

    mail_client = create_email_service(config, config.get("proxy", ""))
    ...
    if hasattr(mail_client, 'api_base'):
