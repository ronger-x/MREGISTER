...
function BrowserAutomationTemplateFields({ template, values, onChange }) {
  ...
  return (
    <div className="full-row browser-template-grid">
      {template.fields.map((field) => {
        const value = values?.[field.key] ?? template.defaults?.[field.key] ?? (field.type === 'boolean' ? false : field.type === 'list' ? [] : '');
        if (field.type === 'boolean') {
          return (
            <label className="checkbox-row field-card field-card--checkbox" key={field.key}>
        ...
        if (field.type === 'list') {
          return (
            <label className="field-card" key={field.key}>
              <span>{field.label}</span>
              <textarea rows="4" value={Array.isArray(value) ? value.join('\n') : ''} placeholder={field.placeholder || ''} onChange={(event) => onChange(field.key, event.target.value.split('\n').map((item) => item.trim()).filter(Boolean))} />
            </label>
          );
        }
        return (
          <label className="field-card" key={field.key}>
            <span>{field.label}</span>
            <input type={field.type === 'number' ? 'number' : 'text'} value={value ?? ''} placeholder={field.placeholder || ''} onChange={(event) => onChange(field.key, field.type === 'number' ? Number(event.target.value || 0) : event.target.value)} />
          </label>
        );
      })}
    </div>
...
export function ConsoleApp() {
  ...
  const cpamcDirtyRef = useRef(false);
  ...
  useEffect(() => {
    cpamcDirtyRef.current = cpamcDirty;
  }, [cpamcDirty]);
  ...
  async function refreshState({ initial = false } = {}) {
    ...
    if (initial || !cpamcDirtyRef.current) {
      setCpamcDraft({
    ...
    await withBusy('task-save', async () => {
      const result = await api('/api/tasks', {
        ...
        body: JSON.stringify({
          ...
          platform_options: taskDraft.platform_options,
        }),
      });
      await refreshState();
  ...
  async function handleRerunTask(task) {
    if (!await confirmAction({
      title: tr('rerun_task') || '重新执行任务',
      message: tr('rerun_task_confirm', { id: task.id }) || `确认从 0 开始重新执行任务 #${task.id}？将克隆原配置并创建新任务。`,
      confirmLabel: tr('rerun_task') || '重新执行',
    })) {
      return;
    }
    await withBusy(`task-rerun-${task.id}`, async () => {
      const result = await api(`/api/tasks/${task.id}/rerun`, { method: 'POST' });
      await refreshState();
      if (result.task_id) {
  ...
  async function handlePreviewSuccessAccounts(task) {
    await withBusy(`task-success-preview-${task.id}`, async () => {
      ...
      await openModal({
        title: `${tr('success_accounts_title')} (${result.count})`,
        message: result.content || tr('success_accounts_empty'),
        confirmLabel: tr('modal_close'),
      });
    });
  }

  async function handleRegenerateOAuthToken(task, account) {
    if (!account?.email) {
      setLoadError(tr('success_accounts_empty'));
      return;
    }
    const email = account.email;
    if (!await confirmAction({
      title: tr('regenerate_oauth_token'),
      message: tr('regenerate_oauth_prompt', { email }),
      confirmLabel: tr('regenerate_oauth_token'),
    })) {
      return;
    }
    await withBusy(`task-regenerate-oauth-${task.id}`, async () => {
      const result = await api(`/api/tasks/${task.id}/success-accounts/oauth`, {
        method: 'POST',
      ...
      <section className="section-card active">
        <article className="panel">
          <form className="grid-two form-grid" onSubmit={handleTaskSubmit}>
            ...
            {currentPlatformSpec.requires_email_credential || taskDraft.platform === 'browser-automation-local' ? (
              <label className="field-card">
            ...
            {taskDraft.platform === 'browser-automation-local' ? (
              <>
                <div className="field-card full-row">
                  <span>{tr('browser_automation_options_title')}</span>
                  <p className="field-tip">{tr('browser_automation_options_desc')}</p>
                </div>
                <div className="field-card full-row">
                  <span>{tr('browser_automation_template_title')}</span>
                  <p className="field-tip">{tr('browser_automation_template_desc')}</p>
                </div>
                <label className="field-card full-row">
                  <span>{tr('field_workflow_template')}</span>
                  <select
                    value={taskDraft.platform_options?.workflow_template || ''}
                    onChange={(event) => {
                      const templateKey = event.target.value;
                      setTaskDraft((current) => {
                        const template = browserTemplates[templateKey] || null;
                        const workflowTemplateValues = template ? { ...(template.defaults || {}) } : {};
                        return {
                          ...current,
                          platform_options: {
                            ...(current.platform_options || {}),
                            workflow_template: templateKey,
                            workflow_template_values: workflowTemplateValues,
                            env_json: buildTemplateEnvJson(templateKey, workflowTemplateValues),
                          },
                        };
                      });
                    }}
                  >
                    <option value="">{tr('field_workflow_template_none')}</option>
                    {Object.entries(browserTemplates).map(([key, item]) => <option key={key} value={key}>{item.label}</option>)}
                  </select>
                </label>
                {selectedBrowserTemplate ? (
                  <div className="field-card full-row">
                    <span>{tr('field_template_fields')}</span>
                    <p className="field-tip">{selectedBrowserTemplate.description}</p>
                  </div>
                ) : null}
                <BrowserAutomationTemplateFields
                  template={selectedBrowserTemplate}
                  values={taskDraft.platform_options?.workflow_template_values || {}}
                  onChange={(key, value) => {
                    setTaskDraft((current) => {
                      const workflowTemplate = current.platform_options?.workflow_template || '';
                      const workflowTemplateValues = {
                        ...(current.platform_options?.workflow_template_values || {}),
                        [key]: value,
                      };
                      return {
                        ...current,
                        platform_options: {
                          ...(current.platform_options || {}),
                          workflow_template_values: workflowTemplateValues,
                          env_json: buildTemplateEnvJson(workflowTemplate, workflowTemplateValues),
                        },
                      };
                    });
                  }}
                />
                <label className="field-card">
                  <span>{tr('field_adapter_path')}</span>
                  <input
                    value={taskDraft.platform_options?.adapter_path || ''}
                    placeholder={tr('field_adapter_path_placeholder')}
                ...
                <label className="field-card">
                  <span>{tr('field_browser_path')}</span>
                  <input
                    value={taskDraft.platform_options?.browser_path || ''}
                    placeholder={tr('field_browser_path_placeholder')}
                    onChange={(event) => setTaskDraft((current) => ({
                      ...current,
                      platform_options: {
                        ...(current.platform_options || {}),
                        browser_path: event.target.value,
                      },
                    }))}
                  />
                </label>
                <label className="checkbox-row field-card field-card--checkbox">
                  <input
                    type="checkbox"
                    checked={Boolean(taskDraft.platform_options?.headless)}
                    onChange={(event) => setTaskDraft((current) => ({
                      ...current,
                      platform_options: {
                        ...(current.platform_options || {}),
                  ...
                  />
                  <span>{tr('field_headless')}</span>
                </label>
                <div className="field-card">
                  <span>{tr('browser_automation_headless_hint')}</span>
                </div>
                <label className="field-card full-row">
                  <span>{tr('field_extra_args')}</span>
                  <input
                    value={taskDraft.platform_options?.extra_args || ''}
                    placeholder={tr('field_extra_args_placeholder')}
                    onChange={(event) => setTaskDraft((current) => ({
                      ...current,
                      platform_options: {
                        ...(current.platform_options || {}),
                ...
                <label className="field-card full-row">
                  <span>{tr('field_env_json')}</span>
                  <textarea
                    rows="10"
                    value={taskDraft.platform_options?.env_json || '{}'}
                    placeholder={tr('field_env_json_placeholder')}
                    onChange={(event) => setTaskDraft((current) => ({
                      ...current,
                      platform_options: {
                        ...(current.platform_options || {}),
            ...
            {taskListMode === 'task' && visibleTask ? (
              <>
                ...
                <div className="task-actions">
                  ...
                  <BusyButton type="button" busy={isBusy(`task-success-preview-${visibleTask.id}`)} onClick={() => handlePreviewSuccessAccounts(visibleTask)}>{tr('preview_success_accounts')}</BusyButton>
                  <button type="button" onClick={() => window.open(`/api/tasks/${visibleTask.id}/success-accounts/download`, '_blank')}>{tr('download_success_accounts')}</button>
                  <BusyButton type="button" busy={isBusy('task-backfill-success-accounts')} onClick={handleBackfillSuccessAccounts}>{tr('extract_history_success_accounts')}</BusyButton>
                ...
                <div className="console-box success-accounts-box">
                  ...
                  {visibleTask.success_accounts_count ? (
                    <div className="success-account-list">
                      {visibleTask.success_accounts.map((account) => (
                        <div className="success-account-item" key={account.email}>
                  ...
                  ) : <pre>{tr('success_accounts_empty')}</pre>}
                </div>
              </>
