...
export function initialTaskDraft(platforms) {
  ...
  return {
    ...
    platform_options: {
      ...
      browser_path: '',
      extra_args: '',
      workflow_template: 'generic-form',
      workflow_template_values: {},
      env_json: '{}',
    },
  };
}

export function buildTemplateEnvJson(templateKey, templateValues) {
  const template = BROWSER_AUTOMATION_TEMPLATES?.[templateKey];
  if (!template) {
    return '{}';
  }
  const payload = { ...(template.defaults || {}) };
...
export function normalizeTaskDraft(draft, platforms, credentials, proxies) {
  ...
  const next = {
    ...
    platform_options: {
      adapter_path: draft.platform_options?.adapter_path || 'browser_automation/runner.py',
      headless: draft.platform_options?.headless ?? true,
      browser_path: draft.platform_options?.browser_path || '',
      extra_args: draft.platform_options?.extra_args || '',
      workflow_template: draft.platform_options?.workflow_template || 'generic-form',
      workflow_template_values: draft.platform_options?.workflow_template_values || {},
      env_json: draft.platform_options?.env_json || '{}',
    },
  };

  if (platform === 'browser-automation-local') {
    next.platform_options.env_json = buildTemplateEnvJson(next.platform_options.workflow_template, next.platform_options.workflow_template_values);
  }
  ...
  if (platform === 'browser-automation-local' && next.email_credential_id && !mailIds.has(String(next.email_credential_id || ''))) {
    next.email_credential_id = '';
  }
